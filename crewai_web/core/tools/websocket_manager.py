...
class WebSocketManager:
    ...
    async def send_progress(
        self, execution_id: str, message: str, step: int = None, total: int = None, status: str = "running"
    ):
        """向指定 execution 的所有连接推送进度"""
        if execution_id not in self.connections:
            logger.debug(f"No WebSocket connections for execution {execution_id}")
            return  # 没有连接，不推送

        event = {"type": "progress", "message": message, "status": status}
