...
class AgentGenerator:
    ...
    async def _find_similar_agent(self, role_type: str, task_context: str, existing_agents: List) -> Optional[any]:
        ...
        # 构建 Agents 列表字符串
        agents_list = "\n".join([
            f"- ID: {agent.id}, Role: {agent.role}, Goal: {agent.goal}"
            for agent in existing_agents
        ])
        
        # 调用 AI 进行语义匹配
        prompt = self.ai_client.load_prompt(
            "generator/agent_match.prompt",
            role_type=role_type,
            task_context=task_context,
            agents_list=agents_list
        )
        
        match_result = await self.ai_client.call(
            prompt=prompt,
            response_model=AgentMatchResult
        )
        
        if match_result.matched and match_result.agent_id:
            # 查找匹配的 Agent 对象
            matched_agent = next(
                (agent for agent in existing_agents if agent.id == match_result.agent_id),
                None
            )
            if matched_agent:
    ...
    async def create_agent(self, role_type: str, task_context: str) -> str:
        ...
        agent_config = await self.ai_client.call(
            ...
            role=role_type  # 传入角色类型，自动注入相关偏好
        )
        ...
        try:
            ...
            if skills_config_dict and skills_config_dict.get("preferred"):
                ...
                logger.info(f"✅ AI recommended {len(skills_config_dict['preferred'])} skills: {skills_config_dict['preferred']}")
            else:
