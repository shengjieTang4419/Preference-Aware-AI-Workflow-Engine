...
class TaskGenerator:
    ...
    async def generate_tasks_plan(self, scenario: str, topic: str, context: Optional[str] = None) -> List[TaskPlan]:
        """
        生成任务规划
        ...
        prompt = self.ai_client.load_prompt(
            "generator/tasks.prompt", topic=topic, scenario=scenario, context_section=context_section
        )

        # 使用 Pydantic 校验
        response = await self.ai_client.call(prompt=prompt, response_model=TasksPlanResponse)
